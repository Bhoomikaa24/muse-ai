...
## Cover Page
 ...
 Muse AI - Intelligent Website Generation System  
 Software Design Specification (SDS)  
 1.0  
 February 26, 2026  
 Project Team  
...
### 1.4 Design Goals
-  Website generation under 10ms
-  4-step wizard, minimal required inputs
...
-  (Restaurant, Home Services, Health & Wellness, etc.)
-  (Modern, Minimal, Luxury, Bold)
-  (no machine learning)
-  for all optional inputs

### 2.2 Architecture Pattern
The system follows a  with clear separation of concerns:

<div class="diagram">

| Layer | Components | Description |
|-------|------------|-------------|
|  | React Components - UI/UX | User interface and visual elements |
| ↓ | | |
|  | State Management, Navigation | Application flow and state control |
| ↓ | | |
|  | Rules Engine, Templates, Generators | Core generation and business rules |
| ↓ | | |
|  | Client-side State, No Database | Temporary data storage |
...
-  Website generation based on inputs
...
|-------|-----------|----------------|
|  | Form Interface | Business details, contact info, design preferences |
| ↓ | | |
|  | React Components | • Landing Page<br>• Create Wizard<br>• Loading Screen<br>• Preview Page |
| ↓ | | |
|  | Navigation & State | • React Router<br>• State Management<br>• Navigation Flow |
| ↓ | | |
|  | Core Processing |  (rules-engine.ts):<br>• validateAndNormalizeInput()<br>• generateWebsite()<br>• selectCTA()<br>• parseServices()<br><br> (templates.ts):<br>• 8 Business Type Templates<br>• Default Values<br>• Business Logic Patterns<br><br> (content-helpers.ts):<br>• Utility Functions<br>• Format Converters<br>• URL Generators |
| ↓ | | |
|  | Generated Website | • Preview Display<br>• Editable Content<br>• Export/Publish |
...
| → | Loading.tsx | <br>• rules-engine.ts<br>&nbsp;&nbsp;• templates.ts<br>&nbsp;&nbsp;• content-helpers.ts<br> Preview |
| → | Preview.tsx | <br>• WebsitePreviewFrame.tsx<br>• EditableText.tsx |
...
#### 4.1.1 Landing Page (Landing.tsx)
 First entry point showcasing platform value proposition
...
- Responsive navigation
 ...
 Routes to `/create` wizard

---

#### 4.1.2 Create Wizard (Create.tsx)
 Multi-step form for gathering business information

 Wizard Pattern with Step Progression



| Step | Section | Fields | Validation |
|------|---------|--------|------------|
| 1 |  | • Business name<br>• Business type (8 options)<br>• Location | Business name required (min 2 characters) |
| 2 |  | • Service list input (comma-separated) | Max 6 services |
| 3 |  | • Phone number<br>• Email address<br>• Physical address<br>• WhatsApp toggle | Email format if provided |
| 4 |  | • Style selection (4 options)<br>• Primary color picker<br>• Style preview | None (all optional) |
...
#### 4.1.3 Loading Screen (Loading.tsx)
 Processes input and generates website



| Component | Action | Description |
|-----------|--------|-------------|
|  | Receives | formData from navigation state |
| |  | 1. Display loading animation (0.5-1s minimum for UX)<br>2. Call validateAndNormalizeInput(formData)<br>3. Call generateWebsite(normalizedInput)<br>4. Navigate to Preview with generated data |
...
```
 ...
 Generation completes in ~5-10ms, delay is for UX only

---

#### 4.1.4 Preview Page (Preview.tsx)
 Display generated website with editing capabilities



| Section | Component | Features |
|---------|-----------|----------|
|  | Action Buttons | • Back to Edit<br>• Edit Mode<br>• Publish |
|  | | |
...
#### 4.2.1 Rules Engine (rules-engine.ts)
 Core generation logic applying business rules
...
##### selectCTA(businessType: string): string
 Choose appropriate call-to-action button text
...
##### parseServices(servicesString: string, businessType: string): string[]
 Parse and validate services input
...
##### getStyleRules(style: string): { layout: Layout; typography: Typography }
 Return design specifications for selected style



| Style | Layout Properties | Typography Properties |
|-------|-------------------|----------------------|
|  | • Spacing: comfortable (16-24px)<br>• Border Radius: rounded (8px)<br>• Shadow: subtle | • Heading Font: Inter, sans-serif<br>• Body Font: Inter, sans-serif<br>• Heading Size: 36px<br>• Body Size: 16px<br>• Line Height: 1.6 |
|  | • Spacing: spacious (24-32px)<br>• Border Radius: sharp (0px)<br>• Shadow: none | • Heading Font: Helvetica, sans-serif<br>• Body Font: Helvetica, sans-serif<br>• Heading Size: 32px<br>• Body Size: 15px<br>• Line Height: 1.8 |
|  | • Spacing: spacious (24-32px)<br>• Border Radius: rounded (8px)<br>• Shadow: pronounced | • Heading Font: Georgia, serif<br>• Body Font: Lato, sans-serif<br>• Heading Size: 42px<br>• Body Size: 17px<br>• Line Height: 1.7 |
|  | • Spacing: comfortable (16-24px)<br>• Border Radius: soft (12px)<br>• Shadow: subtle | • Heading Font: Montserrat, sans-serif<br>• Body Font: Open Sans, sans-serif<br>• Heading Size: 40px<br>• Body Size: 16px<br>• Line Height: 1.6 |
...
#### 4.2.2 Templates System (templates.ts)
 Predefined content patterns for business types
...
```
 ...
 8 (Restaurant, Home Services, Health, Professional, Retail, Beauty, Fitness, Other)
...
The system uses  components built on  primitives:
...
### 8.1 Why Rule-Based Instead of AI?
 ...
 Use deterministic templates and rules instead of AI/ML


1.  Rule-based generation is instant (<10ms vs seconds for AI)
2.  Same inputs always produce same output
3.  No data sent to external servers
4.  No API costs or usage limits
5.  No API downtime or rate limits
6.  Easier to maintain and debug
7.  Users understand what they get
...
### 8.2 Why Client-Side Only?
 ...
 No backend server, all processing in browser


1.  No server deployment or maintenance
2.  Free hosting (static sites)
3.  No network latency
4.  User data never leaves their browser
5.  No server capacity concerns
6.  No database to compromise
...
### 8.3 Why 8 Business Types?
 ...
 Support exactly 8 specific business types


1.  These 8 types cover ~80% of small businesses
2.  Each type gets tailored content patterns
3.  Manageable number of templates
4.  Not overwhelming for users
...
### 8.4 Why 4 Design Styles?
 ...
 Offer exactly 4 design style options


1.  4 options provide variety without analysis paralysis
2.  Each style has distinct personality
3. 
...
### 8.5 Why Optional Inputs with Defaults?
 ...
 Make ALL inputs optional, provide smart defaults


1.  Lower barrier to entry
2.  Users can generate a site with just a business name
3.  Users can add details as needed
4.  Always have valid data to work with
...
### 8.6 Why Limit to 6 Services?
 ...
 Maximum 6 services per business


1.  Beyond 6, service sections become cluttered
2.  Forces businesses to highlight their main offerings
3.  6 services fits well on mobile devices
4.  Users can easily scan 6 items
...
4.  Large component library (shadcn/ui)
...
1.  Reduces complexity significantly
2.  No database hosting costs
3.  No user data stored
4.  Instant generation every time
5.  Websites are output, not stored data
...
### 8.10 Why Template Pattern?
 ...
 Use template pattern with interpolation


1.  Consistent quality
2.  Easy to update templates
3.  Fast string replacement
4.  Easy to unit test
5.  Templates can be extended
...
1.  System doesn't collect or store user data
2.  No analytics or tracking scripts
3.  No cookies or local storage of personal data
...
Muse AI successfully implements a  that:
...
1.  4-step wizard, minimal required inputs
2.  Imperceptible generation time (~5-10ms)
3.  Professional, mobile-responsive output
4.  Deterministic, no external dependencies
5.  Clean architecture, type-safe code
6.  Easy to add business types and styles

### 12.3 Future Enhancements

| Phase | Features | Priority |
|-------|----------|----------|
|  | • Additional business types (Education, Events, Non-profits)<br>• Advanced color schemes (analogous, triadic)<br>• Custom font selection<br>• Multi-page websites | Medium |
|  | • SEO optimization tools<br>• Portfolio/gallery templates<br>• Booking system integration<br>• Contact form generation | High |
|  | • Multi-language support<br>• A/B testing for content<br>• Analytics integration<br>• Custom domain connection | Future |
...
|---------|----------|----------|
|  | Fast generation algorithms | Sub-10ms generation without sacrificing quality |
|  | Smart defaults with options | Easy to use, extensible for advanced users |
|  | Client-side processing | Full features without data collection |
|  | TypeScript implementation | Type safety without productivity loss |
...
|-------|-----------|--------------|-------------|
|  | App.tsx | Main router | Main application component |
| ↓ | | | |
|  | Landing.tsx | None | Entry page |
...
| ↓ | | | |
|  | StepIndicator.tsx | Used by Create.tsx | Progress visualization |
...
| ↓ | | | |
|  | rules-engine.ts | templates.ts, content-helpers.ts | Core generation engine |
...
|  | templates.ts | Business type templates |
