...
Presentation Layer (React Components - UI/UX)
        ↓
Application Layer (State Management, Navigation)
        ↓
Business Logic Layer (Rules Engine, Templates, Generators)
        ↓
Data Layer (Client-side State, No Database)
```
...
User Input (Form Interface)
        ↓
Presentation Layer (Landing → Create → Loading → Preview)
        ↓
Application Layer (React Router, State Management, Navigation)
        ↓
Business Logic Layer
  ├── Rules Engine (validateAndNormalizeInput, generateWebsite, selectCTA, parseServices)
  ├── Templates (8 Business Types, Default Values, Patterns)
  └── Content Helpers (Utilities, Format Converters, URL Generators)
        ↓
Output (Generated Website Preview, Edit, Export, Publish)
```
...
├── Loading.tsx (Depends on: rules-engine.ts, templates.ts, content-helpers.ts)
│   └── Navigates to: Preview
└── Preview.tsx (Depends on: WebsitePreviewFrame.tsx, EditableText.tsx)
```
